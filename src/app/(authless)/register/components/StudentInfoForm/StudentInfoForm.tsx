...
export const StudentInfoForm: React.FC<StudentInfoFormProps> = () => {
    ...
    return (
        <Form layout="vertical" style={{marginTop: 20}} onFinish={handleSubmit(onSubmit)}>
            ...
            <Form.Item label="Universitet" required validateStatus={errors.universityName ? 'error' : ''} help={errors.universityName?.message}>
                <Controller
                    control={control}
                    name="universityName"
                    render={({field}) => (
            ...
            <Form.Item label="İxtisas" required validateStatus={errors.major ? 'error' : ''} help={errors.major?.message}>
                <Controller
                    control={control}
                    name="major"
            ...
            <Flex justify="space-between" gap={8}>
                <Form.Item label="Başlama İli" style={{width: "48%"}} required validateStatus={errors.startYear ? 'error' : ''} help={errors.startYear?.message}>
                    <Controller
                        control={control}
                        name="startYear"
                        render={({field}) => (
                ...
                <Form.Item label="Bitirmə İli" style={{width: "48%"}} required validateStatus={errors.endYear ? 'error' : ''} help={errors.endYear?.message}>
                    <Controller
                        control={control}
                        name="endYear"
                        render={({field}) => (
            ...
            <Form.Item label="Biznes Loqosu" required validateStatus={errors.studentIdFiles ? 'error' : ''} help={errors.studentIdFiles?.message}>
                <Controller
                    name="studentIdFiles"
                    control={control}
                    render={({ field }) => (
